...
function SocialLogoin({ handleKakaoLogin }) {
  return (
    <SignUpFromWrap>
      <SignUpSns>
        <SignUpSnsTitle>SNS 계정으로 간편하게 로그인/회원가입</SignUpSnsTitle>
        <SnsLogo>
          <SignUpSnsListItem>
            <a href="users/auth/facebook">{facebookIconSvg}</a>
            <KakaoLog onClick={handleKakaoLogin}>{kakaoIconSvg}</KakaoLog>
            <a href="users/auth/naver">{naverIconSvg}</a>
          </SignUpSnsListItem>
        </SnsLogo>
      </SignUpSns>
    </SignUpFromWrap>
...
export default SocialLogoin;

const SignUpFromWrap = styled.div``;

const SignUpSns = styled.div`
  display: flex;
...
const SnsLogo = styled.ol`
  ${flexSet('space-evenly', 'center')}
  a {
    margin: 0px 15px;
  }
`;

const SignUpSnsListItem = styled.li`
  margin: 0 10px;
`;

const KakaoLog = styled.button`
